...
//	$Source = $_POST['Source'];
	...
	if($_FILES['Source']['error']>0)
	{
		...
		$sql = "insert into score (number,originalname,chinesename,author,arranger,whenuse,scorestate,level,music,note,updatetime,getsource) values ('$Number','$OriginalName','$ChineseName','$Author','$Arranger','$WhenUse','$ScoreState','$Level','$Music','$Note','$date','$GetSource');";
		$db->query($sql);
		echo "其他資料已新增。";
	...
	{
		$sql = "insert into score (number,originalname,chinesename,author,arranger,whenuse,source,scorestate,level,music,note,updatetime,getsource) values ('$Number','$OriginalName','$ChineseName','$Author','$Arranger','$WhenUse','$FileName','$ScoreState','$Level','$Music','$Note','$date','$GetSource');";
		$db->query($sql);
